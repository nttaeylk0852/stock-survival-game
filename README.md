...
## 11. 중간 점검 개정 (2026-08-16) — §1을 폐기

엔진 묶음 1~6은 서버에 구현됨. 이후 방향:

- **NPC 세력 폐기 (§1 무효)**: 가치/모멘텀/공매도 프로파일은 만들지 않고, 이미 있는 코드는 삭제 예정. 가격은 fair price + 뉴스 + AMM으로 충분.
- **유저 기관 (후순위, 지금 구현 금지)**: 유저 의결권 총합 상한 30%. 기관 권한은 영구가 아니라 월 단위, 기준 미달 박탈, 차순위 제안(거부 가능). 멀티플레이어 이후에만.
- **직장 최소 (잔여)**: 일용직(소액·제약 약함) / 정규직(급여↑, 메인세션 또는 일일 거래와 트레이드오프). 수치는 config. 별도 직업 시뮬 금지.
- **손절·예약주문 (잔여, §7)**: 지정가/손절만. 자동매매 봇 금지(일일캡과 충돌).
- **기각**: 선물(증거금·청산=다른 게임). 가상화폐(이익 루프 희석). 별도 실물시장(원자재→CPI→금리→이익→생존비가 이미 실물).

UI·모바일은 미정. 새 시장을 추가하지 않는다.
- 현재 위치·우선순위·UI 거점의 해석은 docs/PROGRESS.md 하단. 설계 본문(§0~§10)을 대체하지 않음.
